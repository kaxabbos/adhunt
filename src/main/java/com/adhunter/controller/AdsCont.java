...
public class AdsCont extends Attributes {
    ...
    @PostMapping("/add")
    public String AdAddNew(Model model, @RequestParam String name, @RequestParam MultipartFile[] photos, @RequestParam int price, @RequestParam Category category, @RequestParam String description, @RequestParam Type type, @RequestParam Region region, @RequestParam String tel, @RequestParam String origin, @RequestParam String citySale) {
        if (photos != null && !Objects.requireNonNull(photos[0].getOriginalFilename()).isEmpty()) {
            try {
                ...
                ad.setDescription(new AdDescription(category, type, region, description, origin, citySale));
                Users user = getUser();
                user.addAd(ad);
                usersRepo.save(user);
            } catch (Exception e) {
    ...
    @PostMapping("/edit/{id}")
    public String AdEditOld(Model model, @RequestParam String name, @RequestParam MultipartFile[] photos, @RequestParam int price, @RequestParam Category category, @RequestParam String description, @RequestParam Type type, @RequestParam Region region, @RequestParam String tel, @RequestParam String origin, @RequestParam String citySale, @PathVariable Long id) {
        Ads ad = adsRepo.getReferenceById(id);
        String[] result_photos;
        if (photos != null && !Objects.requireNonNull(photos[0].getOriginalFilename()).isEmpty()) {
            try {
        ...
        adDescription.setRegion(region);
        adDescription.setOrigin(origin);
        adDescription.setCitySale(citySale);
